  ...
  const handleSaveRecipe = async (recipeId) => {
    ...
    try {
      const response = await axios.post(
        `${config.BASE_URL}/api/save/${user._id}/${recipeId}`,
        {},
        {
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        <div className="text-white text-center mb-8">
          ...
          <p className="text-sm text-gray-300 mt-2">
            This is the best place to find new and amazing recipes for cooking
            food. We hope you enjoy your time here!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {recipes &&
            recipes.map((recipe, index) => (
              <div
                key={index}
                className="group block rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition duration-300"
              >
                <img
                  src={recipe.imgUrl}
                  alt={recipe.title}
                ...
                <div className=" sm:grid-cols-1 flex justify-between items-center p-4">
                  <button
                  ...
                  <div className=" md:grid-cols-1 gap-4 p-4">
                    <button
                      className={
                        user
                          ? "bg-green-500 hover:bg-green-600 text-sm text-white py-2 px-4 rounded-md transition duration-300"
                          : "bg-gray-500 text-white py-2 px-4 rounded-md"
                      }
                      onClick={() => handleSaveRecipe(recipe._id)}
                      disabled={!user}
                    >
                      {!user ? "Login to save recipe" : "Save"}
                    </button>
                  </div>
                </div>
              </div>
